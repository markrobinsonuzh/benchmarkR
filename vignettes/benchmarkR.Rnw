...
the set of standard plots that many methodologists look at with calibration information.  
...
\Rpackage{benchmarkR} is an R/Bioconductor package designed to evaluate and visualize classification performance for reference datasets.  Generally speaking, the package of \Rpackage{benchmarkR} can be divided into two major parts:\\
\begin{enumerate}
...
The function \Rfunction{SimResults} is just the constructor to build a \Robject{SimResults} object. It can be invoked as below:
\begin{example}[H]                 
\centering
...
To give a practical example, we have pre-packaged the simulation results from Zhou et al. (2014) \cite{Zhou2014a} to
highlight the construction of an \Robject{SimResults} object (see also \Rfunction{help("Pickrell")}): \\
% 
