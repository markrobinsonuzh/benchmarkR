...
The typical usage of the \Rpackage{benchmarkR} package would involve p-values, in order to do a calibration assessment.  More generally, the package will happily take ``scores''.  An example of this is given in \fixme{PUT SECTION HERE.}.
