...
visualize the performance of \fixme{explain classification method}, such as those common in statistical genomics.
...
\begin{center}
   \begin{tabular}{ | l | }
      ...
      \tabitem BH: Benjamini \& Hochbergi \cite{Benjamini1995}\\       
      \hline 
\end{tabular}
\end{center}
\vspace{1em}
...
\begin{itemize}
  ...
  \item \Robject{padj}: a vector or matrix containing the adjust p-values (optional).
  \item \Robject{labels}: a numeric vector indicating positive with a 1, and negative with a 0.
\end{itemize}
...
In this case,  adjust p-values (\Robject{padj}) is missing. Internally, p-values will be adjusted using a method of choice (default is Benjamini-Hochberg \cite{Benjamini1995}, \Robject{padjMethod="BH"}, but others from \Rfunction{p.adjust} can be used). 
...
Note that \Robject{pval} (and \Robject{padj}) can be a vector or matrix (for a single method or multiple methods). 
If a matrice is used, each column should represent a score (p-value) a classification method and 
...
\Rfunction{benchmarkR} will  produce a set of performance assessment plots for benchmarking.  A 3-panel plot is created with: an ROC curve (\Rfunction{rocX}), a false discovery (FD) plot (\Rfunction{fdX}) and a true-positive-rate versus achieved false-discovery plot (\Rfunction{powerFDR}).  All plots show the augmented calibration information.
...
The constructor function \Rfunction{powerFDR} will produce an S4 object of \Rclass{powerFDR}. A typical usage for an object of \Rclass{SimResults} is:\\
...
Most of the graphical parameters such as `col', `cex', `pch', etc. from \Rfunction{par}, can be directly passed to \Rfunction{plot} for the power-FDR plots. `pointType', letter indicating how power-FDR values should be plotted: \Robject{"b"} for both points and lines; \Robject{"p"} for points only; and \Robject{"l"} for lines only. `col.line' and `lwd.line', `col' and `lwd' of line connecting power-FDR points, if `pointType' is either \Robject{"b"} or \Robject{"l"}. `lwd.threshold', `lty.threshold' and `col.threshold' are `wd', `lty' and `col' referred to the lines drawn for the threshold.
...
A method's FDR is considered to be controlled if its achieved FDR falls below the threshold; 
...
\Rfunction{fdX} plots cumulative number of false discoveries versus the number of top ranked features with an X point marking the location of the method's FDR (at a chosen level).The constructor function \Rfunction{fdX} will produce an S4 object of \Robject{fdX}. \\
