...
\begin{enumerate}
  \item \Robject{SimResults}:  a S4 class object to store the p-values, true labels and other necessary values for further analysis and evaluation.
  \item \Rfunction{benchmarkR}: a method to produce a visualized benchmark result including a 3-panel plot of a ROC curve (\Robject{rocX}), a false discovery (FD) plot (\Robject{fdX}) and a true-positive-rate versus achieved false-discovery plot(\Robject{powerFDR}).
\end{enumerate}
...
In this case,  adjust p-values (\Robject{padj}) is missing. Internally, P-values will be adjusted using a method of choice (default is Benjamini-Hochberg, adjMethod="BH", but others from \Rfunction{p.adjust} can be used). 
...
If matrices are used, each column should represent a score (p-values) a classification model and 
...
f <- as.factor(sample(1:4, nrow(Pickrell$pval), replace=TRUE))
